...
- 公告采用草稿优先流程：创建接口只创建 `draft`；立即发布或定时发布必须通过显式发布动作，未来时间进入 `scheduled`。
- 撤销消息保留受控占位：收件箱和详情可返回消息标识、类型、标题、撤销/过期状态及时间等元数据，但不得返回正文、图片或外链；不可见请求仍返回统一资源不可用错误。
- 组织级消息分类不预置默认记录；组织必须先创建并启用分类，消息创建或发布时按组织分类编码校验。
- 本 Change 的外部邮件范围仅为邮箱验证 SMTP，以及供未来邮件/短信 Consumer 使用的进程内 Messaging Projection Contract；不发送业务消息邮件，不实现第三方通知适配器。
- WebSocket Gateway 对每条连接使用有界写入队列；队列满或写入超过 5 秒即关闭连接，客户端通过游标恢复，不得无限等待或无限积压。
