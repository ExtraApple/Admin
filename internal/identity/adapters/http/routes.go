...
func Routes(service *application.Service, users *application.UserService, contextService *application.ContextService, avatars *application.AvatarService, captcha CaptchaGenerator, avatarMaxSizeMB int, logoutExpires ...time.Duration) []routecatalog.Descriptor {
	...
	return []routecatalog.Descriptor{
		...
		identityRoute(http.MethodPut, "/api/user/password", "Change Password", routecatalog.Authenticated, handler.changePassword, identity.ChangePasswordRequest{}, messageResponse{}),
		...
		identityRoute(http.MethodPost, "/api/user/logout", "Logout", routecatalog.Authenticated, handler.logout, nil, messageResponse{}),
		...
		permissionIdentityRoute(http.MethodDelete, "/api/admin/users/:id", "Delete User", "admin.users.id.delete", handler.deleteUser, nil, messageResponse{}),
		permissionIdentityRoute(http.MethodPut, "/api/admin/users/:id/status", "Toggle User Status", "admin.users.id.status.put", handler.toggleStatus, nil, messageResponse{}),
		permissionIdentityRoute(http.MethodPut, "/api/admin/users/:id/kick", "Kick User", "admin.users.id.kick.put", handler.kickUser, nil, messageResponse{}),
	}
}
...
func identityRoute(method, path, name string, access routecatalog.AccessLevel, handler gin.HandlerFunc, request, response any) routecatalog.Descriptor {
	...
	return routecatalog.Descriptor{Method: method, Path: path, Access: access, Handler: handler, Name: name, Group: "identity", DefaultAuditCategory: "identity", OpenAPI: routecatalog.Operation{Summary: name, Request: requestBody, Responses: map[int]routecatalog.Response{http.StatusOK: {Description: "success", Kind: routecatalog.JSONBody, Schema: reflect.TypeOf(response)}}}}
}
...
func (handler *routeHandler) captchaHandler(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": 200, "data": gin.H{"captcha_id": id, "captcha_img": image}})
}
...
func (handler *routeHandler) register(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": 200, "msg": "注册成功", "data": identity.UserInfoFromDomain(user)})
}
...
func (handler *routeHandler) login(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": 200, "msg": "登录成功", "data": identity.LoginResponse{AccessToken: result.AccessToken, RefreshToken: result.RefreshToken, User: identity.UserInfoFromDomain(result.User)}})
}

func (handler *routeHandler) refresh(c *gin.Context) {
	var request identity.RefreshTokenRequest
	if err := c.ShouldBindJSON(&request); err != nil {
		c.JSON(http.StatusUnauthorized, gin.H{"code": 401, "msg": application.ErrRefreshTokenInvalid.Error()})
		return
	}
	...
	c.JSON(http.StatusOK, gin.H{"code": 200, "msg": "刷新成功", "data": identity.RefreshTokenResponse{AccessToken: pair.AccessToken, RefreshToken: pair.RefreshToken}})
}
...
func (handler *routeHandler) getInfo(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": 200, "msg": "success", "data": identity.UserInfoFromDomain(user)})
}

func (handler *routeHandler) updateSelf(c *gin.Context) {
	var request identity.UpdateSelfRequest
	if err := c.ShouldBindJSON(&request); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"code": 400, "error_code": "REQUEST_INVALID", "msg": err.Error()})
		return
	}
	...
	if err != nil {
		if err == application.ErrAvatarFieldNotWritable {
			c.JSON(http.StatusBadRequest, gin.H{"code": 400, "error_code": "AVATAR_FIELD_NOT_WRITABLE", "msg": err.Error()})
			return
		}
		badRequest(c, err)
		return
	}
	c.JSON(http.StatusOK, gin.H{"code": 200, "msg": "修改成功", "data": identity.UserInfoFromDomain(user)})
}
...
func (handler *routeHandler) listUsers(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": 200, "data": identity.UserListResponse{List: list, Total: result.Total, Page: result.Page, Size: result.Size}})
}
...
func (handler *routeHandler) toggleStatus(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": 200, "msg": message})
}
...
func (handler *routeHandler) initialContext(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": 200, "data": identity.InitialContextResponse{User: identity.UserInfoFromDomain(result.User), Roles: result.Roles, Permissions: result.Permissions, Menus: identity.MenuDetailsFromDomain(result.Menus)}})
}
...
func (handler *routeHandler) uploadAvatar(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": 200, "msg": "头像上传成功", "data": identity.UserInfoFromDomain(result.User)})
}
...
func (handler *routeHandler) restoreDefaultAvatar(c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{"code": 200, "msg": "已恢复默认头像", "data": identity.UserInfoFromDomain(user)})
}
...
func writeAvatarError(c *gin.Context, err error) {
	...
	status, message := avatarHTTPError(code)
	c.JSON(status, gin.H{"code": status, "error_code": code, "msg": message})
}

func avatarHTTPError(code string) (int, string) {
	switch code {
	case "REQUEST_INVALID":
		return http.StatusBadRequest, "请求参数不正确"
	case "AVATAR_FIELD_NOT_WRITABLE":
		return http.StatusBadRequest, "头像只能通过专用接口修改"
	case "UPLOAD_BODY_INVALID":
		return http.StatusBadRequest, "上传请求格式不正确"
	case "UPLOAD_BODY_TOO_LARGE":
		return http.StatusRequestEntityTooLarge, "上传请求超过大小限制"
	case "UPLOAD_FILE_MISSING":
		return http.StatusBadRequest, "请选择文件"
	case "UPLOAD_MULTIPLE_FILES":
		return http.StatusBadRequest, "每次只能上传一个文件"
	case "FILE_EMPTY":
		return http.StatusBadRequest, "文件不能为空"
	case "FILE_TOO_LARGE":
		return http.StatusRequestEntityTooLarge, "文件超过大小限制"
	case "FILE_NAME_INVALID":
		return http.StatusBadRequest, "文件名不合法"
	case "FILE_TYPE_NOT_ALLOWED":
		return http.StatusUnsupportedMediaType, "文件类型不受支持"
	case "FILE_TYPE_MISMATCH":
		return http.StatusUnsupportedMediaType, "文件类型不一致"
	case "FILE_ENCODING_INVALID":
		return http.StatusUnprocessableEntity, "文件编码无效"
	case "FILE_CONTENT_INVALID":
		return http.StatusUnprocessableEntity, "文件内容无效"
	case "IMAGE_DIMENSION_LIMIT":
		return http.StatusUnprocessableEntity, "图片尺寸超过限制"
	case "IMAGE_DECODE_INVALID":
		return http.StatusUnprocessableEntity, "图片无法完整解码"
	case "STORAGE_OBJECT_NOT_FOUND":
		return http.StatusNotFound, "文件对象不存在"
	case application.AvatarCodeStorageUnavailable:
		return http.StatusServiceUnavailable, "文件存储服务暂时不可用"
	case application.AvatarCodePersistenceFailed:
		return http.StatusInternalServerError, "数据保存失败"
	default:
