...
func TestAnnouncementServiceEditsPublishedCopyAndPreservesReadState(t *testing.T) {
	...
	if err != nil || edited.Status != domain.MessageStatusPublished || edited.AggregateVersion != 2 || edited.Title != "Edited" || edited.CategoryID == created[0].CategoryID {
	...
	if err := db.Last(&outbox).Error; err != nil || outbox.EventName != domain.EventNameMessageEdited || outbox.AggregateVersion != 2 {
