...
func TestBroadcastServiceBindsReferencedImagesInMessageTransaction(t *testing.T) {
	db := testutil.OpenIsolatedSQLite(t)
	if err := db.AutoMigrate(messaginggorm.Models()...); err != nil {
		t.Fatalf("migrate Messaging models: %v", err)
	}
	...
	copies, err := service.CreateBroadcast(context.Background(), application.CreateBroadcastRequest{ActorID: 7, CategoryCode: "notice", Title: "System update", Markdown: "Body", Targets: []application.DynamicAudienceTarget{{OrganizationID: 10, Type: domain.AudienceTypeOrganization}}, ImageIDs: []uint{91}})
