...
func New(ctx context.Context, conf platformconfig.Config, resources Resources, options Options) (*Application, error) {
	...
	authorizationApplication := newAuthorizationService(resources)
	routeSource := &catalogRouteSource{}
	modules := newNavigationComposition(resources, authorizationApplication, routeSource)
	identityModule := newIdentityComposition(resources, conf, authorizationApplication, identityNavigation{navigation: modules.Navigation})
	auditModule := newAuditComposition(resources, conf, identityModule.directory)
	navigationRoutes := navigationmodule.Routes(modules.Navigation)
	apiRoutes := apihttp.Routes(modules.APIService)
	organizationRoutes := organizationDescriptorsWithVisibility(resources, authorizationOrganizationVisibility{authorization: authorizationApplication})
