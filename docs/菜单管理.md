...
> **文档定位**: 已实现模块的实现说明。当前菜单树、角色菜单、用户菜单和路由同步行为以 `openspec/specs/menu-management/spec.md` 为准；本文保留接口示例和实现记录。

## 当前状态

> **状态**: 已实现菜单树、菜单 CRUD、角色绑定菜单、前端路由同步、菜单权限码和按钮权限字段。
...
type Menu struct {
    ...
    PermissionCode string `gorm:"type:varchar(100);comment:权限码"`
...
- `type=1`：目录，一般不填 `permission_code`
- `type=2`：页面菜单，可按页面访问权限填写 `permission_code`
- `type=3`：按钮权限，通常必须填写 `permission_code`，`path` 可以为空
...
`/api/user/context` 返回用户初始上下文时，会返回当前用户可见的菜单树：

- `admin` 角色默认返回全部启用菜单。
- 普通用户只返回角色绑定菜单。
- 如果菜单 `permission_code` 为空，则只要角色绑定了该菜单即可显示。
- 如果菜单 `permission_code` 不为空，则用户必须拥有对应权限码。
- 如果父级目录本身没有权限码，但下面存在可见子菜单或按钮，父级目录会保留。
...
```go
func GetMenuTree() ([]dto.MenuInfo, error)
func CreateMenu(req dto.CreateMenuReq) (*dto.MenuInfo, error)
func UpdateMenu(menuID uint, req dto.UpdateMenuReq) (*dto.MenuInfo, error)
func DeleteMenu(menuID uint) error
func AssignMenusToRole(roleID uint, menuIDs []uint) error
func GetRoleMenus(roleID uint) ([]dto.MenuInfo, error)
func GetUserMenus(userID uint) ([]dto.MenuInfo, error)
func SyncMenus(routes []dto.SyncMenuItem) (created int, error)
```
