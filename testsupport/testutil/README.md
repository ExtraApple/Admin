...
```powershell
.\testsupport\testutil\run-sqlite-gate.ps1
```
...
.\testsupport\testutil\run-mysql-gate.ps1
```
...
认证、Refresh Token 和黑名单的真实 Redis 验收：

```powershell
$env:ADMIN_TEST_REDIS_ADDR = "127.0.0.1:6379"
.\testsupport\testutil\run-redis-gate.ps1
```

可选设置 `ADMIN_TEST_REDIS_PASSWORD` 和 `ADMIN_TEST_REDIS_DB`；门禁默认使用数据库 15。

## MinIO 强制门禁

对象存储 Put/Open/Stat/List/Delete 真实链路验收：

```powershell
$env:ADMIN_TEST_MINIO_ENDPOINT = "127.0.0.1:9000"
$env:ADMIN_TEST_MINIO_ACCESS_KEY = "minioadmin"
$env:ADMIN_TEST_MINIO_SECRET_KEY = "minioadmin"
$env:ADMIN_TEST_MINIO_SECURE = "false"
.\testsupport\testutil\run-minio-gate.ps1
```

MinIO 门禁创建唯一测试 bucket，并在测试结束时删除对象和 bucket；缺少连接配置或真实服务不可达时失败。
